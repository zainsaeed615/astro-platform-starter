/**
 * Subtle bat animation for static hero images.
 * Keeps the full scene intact while only the detected bat silhouettes move.
 */
export class BatAnimation {
    ...
    #detectBats(width, height) {
        ...
        for (let y = 0; y < scanHeight; y++) {
            for (let x = 0; x < width; x++) {
                ...
                const isDark = value <= this.options.luminanceThreshold;
                const isContrast = value <= localAvg - this.options.contrastDelta;
    ...
    #fallbackBats(width, height) {
        ...
        return presets.map(([rx, ry, rw, rh], index) =>
            this.#createBat(
                Math.round(rx * width),
                Math.round(ry * height),
                Math.round(rw * width),
                Math.round(rh * height),
                index
            )
        );
    }
...
/**
 * Boot animation on a canvas with a data-image attribute.
 * @param {string} selector
 */
export function initBatAnimation(selector = '[data-bat-animation]') {
